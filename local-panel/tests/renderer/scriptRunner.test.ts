...
const noopSend: SendRequestFn = async () => ({ status: 200, headers: {}, body: "" });
...
describe("runPreScript()", () => {
  it("returns the original request unchanged when script is empty", async () => {
    const req = makeReq();
    const result = await runPreScript("", req, null, noopSend);
  ...
  it("allows the script to read lp.request.url", async () => {
    ...
    const result = await runPreScript("lp.request.url;", req, null, noopSend);
  ...
  it("allows the script to mutate lp.request.url", async () => {
    ...
    const result = await runPreScript("lp.request.url = 'http://new.localhost';", req, null, noopSend);
  ...
  it("does NOT mutate the original ScriptRequest object (works on a copy)", async () => {
    ...
    await runPreScript("lp.request.url = 'http://mutated.localhost';", copy, null, noopSend);
  ...
  it("returns error string on script runtime error", async () => {
    const result = await runPreScript("throw new Error('oops');", makeReq(), null, noopSend);
    expect(result.error).toContain("oops");
  });

  it("returns error string on script syntax error", async () => {
    const result = await runPreScript("((((broken syntax", makeReq(), null, noopSend);
    expect(result.error).toBeDefined();
  });

  it("lp.sendRequest is not available in the vm sandbox (errors gracefully)", async () => {
    const send = vi.fn<SendRequestFn>().mockResolvedValue({ status: 200, headers: {}, body: "ok" });
    const result = await runPreScript(
...
describe("runPostScript()", () => {
  ...
  it("returns no error for an empty script", async () => {
    const result = await runPostScript("", resp, null, noopSend);
    expect(result.error).toBeUndefined();
  });
  ...
  it("returns error on script runtime error", async () => {
    const result = await runPostScript("throw new Error('post-fail');", resp, null, noopSend);
    expect(result.error).toContain("post-fail");
  });

  it("lp.sendRequest is not available in the vm sandbox (errors gracefully)", async () => {
    const send = vi.fn<SendRequestFn>().mockResolvedValue({ status: 200, headers: {}, body: "secondary" });
    const result = await runPostScript(
...
describe("makeSendRequest()", () => {
  const baseReq: SavedRequest = {
    id: "req-auth",
    name: "Auth",
    method: "POST",
  ...
  it("throws if the requestId is not found", async () => {
    const send = makeSendRequest([], {});
    await expect(send("nonexistent")).rejects.toThrow(/no request with id/);
  });

  it("resolves env vars in url, headers, and body before calling replayRequest", async () => {
    const send = makeSendRequest([baseReq], { USER: "alice" });
    await send("req-auth");
    const mock = (globalThis as any).window.api.replayRequest as ReturnType<typeof vi.fn>;
    expect(mock).toHaveBeenCalledWith(
      "POST",
      "http://auth.localhost/login",
      expect.objectContaining({ "content-type": "application/json" }),
      expect.any(String),
    );
    const bodyArg: string = mock.mock.calls[0][3];
    const decoded = atob(bodyArg);
    expect(decoded).toContain("alice");
  });

  it("does NOT run pre/post scripts on the called request", async () => {
    const reqWithScript: SavedRequest = {
      ...baseReq,
      preScript: "throw new Error('pre-script should not run');",
      postScript: "throw new Error('post-script should not run');",
    };
    const send = makeSendRequest([reqWithScript], {});
    await expect(send("req-auth")).resolves.toEqual({
      status: 200,
      headers: { "content-type": "application/json" },
      body: '{"token":"xyz"}',
    });
  ...
  it("decodes the base64 response body to plain text", async () => {
    const send = makeSendRequest([baseReq], {});
    const result = await send("req-auth");
    expect(result.body).toBe('{"token":"xyz"}');
  });
});
