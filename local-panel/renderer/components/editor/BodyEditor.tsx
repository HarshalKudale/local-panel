...
export default forwardRef<BodyEditorHandle, Props>(function BodyEditor({
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="flex items-center border-b border-border/40 flex-shrink-0 bg-bg0/20">
        {/* Mode selector (left) */}
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-[10px] font-medium transition-colors cursor-pointer disabled:cursor-default ${
                mode === m
                  ...
                  : "text-text-dim hover:text-text-base hover:bg-bg2/50"
              }`}
            >
              {MODE_LABELS[m]}
            </button>
        ...
        <div className="flex items-center gap-3 px-3 py-1.5 flex-1 justify-between">
          ...
          <div className="flex items-center gap-3">
            {syntaxError && <span className="text-[10px] text-red font-mono">{syntaxError}</span>}
            {!readOnly && mode === "json" && (
              <button
      ...
      {mode === "none" ? (
        ...
        <div className="flex flex-col flex-1 overflow-y-auto">
          ...
          {formPairs.map((pair) => (
            <div key={pair.id} className="flex items-stretch border-b border-border/25 last:border-0 group hover:bg-bg2/30 transition-colors">
