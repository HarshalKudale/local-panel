  ...
  return (
    <div className="relative flex-shrink-0" onMouseDown={(e) => e.preventDefault()}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 px-2 py-1 rounded border border-border bg-card hover:bg-surface-2 text-muted-foreground hover:text-signal text-[10px] font-mono transition-colors cursor-pointer"
      >
        <span className="text-signal/70">{"{{"}</span>
        <span className="text-muted-foreground">{env.name.slice(0, 10)}</span>
        <span className="text-signal/70">{"}}"}</span>
      </button>
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full mt-1 right-0 z-50 bg-card border border-border rounded-md shadow-2xl py-1 min-w-[200px] max-h-64 overflow-y-auto">
            ...
            {env.variables.filter((v) => v.key.trim()).map((v) => (
              ...
              >
                <span className="font-mono text-signal font-semibold">{"{{" + v.key + "}}"}</span>
                <span className="text-muted-foreground text-[10px] truncate max-w-[100px] group-hover:text-foreground">
